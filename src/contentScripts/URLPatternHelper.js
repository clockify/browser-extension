        ...
        if(patternHostnameElements.length === linkHostnameElements.length){
            for(let i = 0; i< patternHostnameElements.length; i++){
                if(patternHostnameElements[i] !== "*"){
                    if(patternHostnameElements[i] !== linkHostnameElements[i]) isMatching=false;
                }
            }
        }else{
            isMatching=false;
...
function isMatchingURL(pattern, url){
    ...
    //returns first match
    return !!(urlPatterns.find(pattern => matchPatternAndLink(parseURLString(pattern), urlInfo)))
}
