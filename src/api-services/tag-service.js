	...
	static async getOrCreateTags(tagNames) {
		...
		for (const tagName of tagNames) {
			...
			if (t) {
				...
				if (!canCreateTags) {
					await localStorage.setItem('createTagsError', true);
				}
				const { data: tag, status } = await this.createTag({ name: tagName });
		...
		if (notifyCanNotreateObjects) message += "\n Integrations can't create tags.";
		if (forceTags && tags.length !== tagNames.length) message += '\n Tags are required.';
