...
function parseTimeWithSeparator(totalTime) {
    if (/^\d\d:\d\d(a|p|am|pm)$/i.test(totalTime) || /^\d:\d\d(a|p|am|pm)$/i.test(totalTime) ||
        /^\d\d:\d\d (a|p|am|pm)$/i.test(totalTime) || /^\d:\d\d (a|p|am|pm)$/i.test(totalTime)) {
        ...
        let totalTimeWithoutLetters =
            totalTime.toLowerCase().replace('a', '').replace('p', '').replace('m', '');
        let hours = '00';
        let minutes = '00';
...
function convertTotalTimeTo24HourDisplay(totalTime) {
    ...
    if (isPM(totalTime)) {
        ...
        result = (Number(result.substring(0, result.indexOf(':'))) + 12).toString()
