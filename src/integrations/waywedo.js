(() => {
	...
	//entire checklist
	clockifyButton.render('#maincontent .checklist-buttons>.pull-right', { observe: true }, (elem) => {
	...
	clockifyButton.render('.fullscreen-open.checklist-open .navbar-fullscreen #navbuttons>div', { observe: true }, (elem) => {
		// Unable to filter out with :not(.clockify) because full screen mode messes with the DOM
		if (elem.getElementsByClassName(checklistWrapperClass).length) {
			return;
		}
	...
	//step content
	clockifyButton.render('#stepcontent .article-heading+.article-meta:not(.clockify)', { observe: true }, (elem) => {
