...
export class ProjectService extends HttpWrapperService {
	...
	async getProjectsByIds(projectIds) {
		...
		const projectUrl = `${baseUrl}/workspaces/${activeWorkspaceId}/projects/ids`;
		const body = {
			ids: projectIds,
		};
	...
	dopuniNonFavorites(alreadyIds, projectUrl, data, page, pageSize, forceTasks) {
		...
		return super
			...
			.then((response) => {
				...
				return this.dopuniNonFavorites(
					alreadyIds,
					projectUrl,
					data,
					page + 1,
	...
	async getProjectTasksWithFilter(projectId, filter, page) {
		...
		const baseUrl = await localStorageService.get('baseUrl');
		const projectUrl =
			`${baseUrl}/workspaces/${activeWorkspaceId}/project-picker/` +
	...
	async getLastUsedProject(forceTasks) {
		...
		const baseUrl = await localStorageService.get('baseUrl');
		const url = `${baseUrl}/workspaces/${activeWorkspaceId}/projects/lastUsed?type=PROJECT${
