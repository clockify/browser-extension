...
export class WorkspaceService extends HttpWrapperService {
	...
	async getPermissionsForUser() {
		...
		return super
			.get(workspacePermissionsUrl, addToken)
	...
	async getWorkspaceSettings() {
		...
		const baseUrl = await localStorageService.get('baseUrl');
		const workspaceSettingsUrl = `${baseUrl}/workspaces/${activeWorkspaceId}`;
	...
	async getWasRegionalEverAllowed() {
		...
		const baseUrl = await localStorageService.get('baseUrl');
		const workspaceRegionalUrl = `${baseUrl}/workspaces/${activeWorkspaceId}/payments/was-regional-ever-allowed`;
