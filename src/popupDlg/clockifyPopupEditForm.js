...
var ClockifyEditForm = class {
	...
	get isForceTasks() {
		return this.wsSettings.forceTasks;
	}
	get isProjectFavorites() {
		return this.wsSettings.projectFavorites;
	}

	get hideBillable() {
		return (
			!this.wsSettings.activeBillableHours ||
			(this.wsSettings.onlyAdminsCanChangeBillableStatus &&
				!_clockifyPopupDlg.isOwnerOrAdmin)
		);
	}

	get isProjectRequired() {
		return this.state.projectRequired;
	}
	get isTaskRequired() {
		return this.state.taskRequired;
	}
	get isTagRequired() {
		return this.state.tagsRequired;
	}

	get SelectedProject() {
		return this.state.timeEntry.project;
	}
	get SelectedProjectId() {
		return this.state.timeEntry.projectId;
	}
	get SelectedTask() {
		return this.state.timeEntry.task;
	}
	get SelectedTaskId() {
		return this.state.timeEntry.taskId;
	...
	create(msg) {
		...
		// project
		const divProjectDown = _clockifyProjectList.create(
		...
		// tags
		const divTagDown = _clockifyTagList.create(
			{ ...this.state.timeEntry, tags: this.state.tags },
			editForm
		);
				...
				customFieldValues.forEach((customField) => {
					...
					if (projectDefaultValues && projectDefaultValues.length > 0) {
						const projectEntry = projectDefaultValues.find(
							(x) => x.projectId === timeEntry.projectId
						);
	...
	onChangeProjectRedrawCustomFields() {
		...
		customFieldValues.forEach((customField) => {
			...
			const cf = this.customFields.find(
				(cf) => cf.customFieldId === wsCustomField.id
			);
			if (status === 'VISIBLE') {
				if (!cf) {
					this.addCustomField({
	...
	createCloseDlg() {
		...
		divCloseDlg.innerHTML =
			`<img id='clockifyCloseDlg' src='${aBrowser.runtime.getURL(
				'assets/images/closeX.svg'
			)}' style='width:14px; height:14px;' alt='Close'` +
	...
	createMsg(msg) {
		...
		span.innerHTML =
			msg +
			`<img id='clockifyCloseMsg' src='${aBrowser.runtime.getURL(
				'assets/images/closeX.svg'
			)}' style='width:12px; height:12px;' alt='Close'` +
	...
	createManualModeHeader() {
		...
		manualHeaderContainer.classList.add(
			'clockify-manual-entry-header-container'
		);
	...
	get descriptionContent() {
		...
		return (
			`<textarea id='clockifyTextareaDescription' type='text' class='clockify-edit-form-description' placeholder='${clockifyLocales.DESCRIPTION_LABEL}'>` +
			desc +
			'</textarea>'
		);
	...
	createDescription() {
		...
		divDesc.innerHTML =
			`<div class='clockify-description-textarea${
				this.descRequired ? ' required' : ''
			}' style='padding:0'>` +
	...
	get billableContent() {
		const { timeEntry } = this.state;
		return (
			`<span class='clockify-edit-form-checkbox${
				timeEntry.billable ? ' clockify-checked' : ''
			}' tabIndex='0' >` +
			`<img src='${aBrowser.runtime.getURL(
				'assets/images/checked.png'
			)}' class='edit-form-billable-img${
				timeEntry.billable ? '' : '-hidden'
			}' />` +
			'</span>' +
		...
		divButtons.innerHTML =
			...
			`<button id='clockifyButtonDone' class='${
				descRequired || projectRequired || taskRequired || tagsRequired
					...
					: 'clockify-edit-form-done'
			}'>${clockifyLocales.DONE_LABEL}` +
			'</button>' +
			'</div>' +
			'</div>';
	...
	onClickedCFPopup(target) {
		...
		for (var i = 0; i < customFields.length; i++) {
			...
			if (['DROPDOWN_SINGLE', 'DROPDOWN_MULTIPLE'].includes(customField.type)) {
				if (customField.onClickedCFPopup(target)) return true;
			}
		}
		return false;
	}
			...
			case 'spanClockifyBillable':
				if (this.state.manualMode) {
					this.editBillableManualMode().then(() => {
						$('#spanClockifyBillable', this.editFormElem).innerHTML =
							this.billableContent;
					});
				} else {
					this.editBillable();
							...
							(response) => {
								...
								if (status !== 200) {
									...
									if (status === 400) {
										...
										// time entry creation
										alert(
											"Can't log time without project/task/description or tags."
										);
									}
								} else {
									...
									this.checkRequiredFields();
									if (this.state.descRequired) this.shakeDescription();
								}
							}
						);
						//e.stopPropagation();
                    ...
                    */
						...
						if (_clockifyProjectList.projectFromList)
							this.state.timeEntry.billable =
								_clockifyProjectList.projectFromList.billable;
						if (!this.hideBillable)
							$('#spanClockifyBillable', this.editFormElem).innerHTML =
								this.billableContent;
						//_clockifyProjectList.mapSelectedProject();
	...
	get tagIds() {
		return this.state.tags ? this.state.tags.map((it) => it.id) : [];
	}
