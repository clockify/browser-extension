...
var ClockifyTagList = class {
	...
	get HeaderContent() {
		...
		let title =
			tags.length > 1
				? `${clockifyLocales.TAGS}:\n`
				: `${clockifyLocales.TAG}: `;

		const arr = tags.map((tag, index, list) => {
			const name = this.encoded(tag.name);
			title += name + '\n';
			return (
				"<span class='clockify-tag-list-selected-item'>" +
				name +
				(index < list.length - 1 ? ', ' : '') +
				'</span>'
			);
		...
		const str =
			...
			` class='${
				isOffline
					? 'clockify-tag-list-button-offline'
					: this.editForm.isTagRequired
					? 'clockify-tag-list-button-required'
					: 'clockify-tag-list-button'
			}' title="${title}">` +
			`<span` +
			...
			(arr.length === 0
				? "<span class='clockify-tag-list-add'>" +
				  (this.editForm.isTagRequired
						? `${clockifyLocales.ADD_TAGS} ${clockifyLocales.REQUIRED_LABEL}`
						: clockifyLocales.ADD_TAGS) +
				  '</span>'
			...
			`<img id='imgClockifyDropDownTags' src='${aBrowser.runtime.getURL(
				img
			)}'` +
	...
	open() {
		aBrowser.storage.local.get(['timeEntryInProgress'], (result) => {
			...
			this.getElem('#imgClockifyDropDownTags').src = aBrowser.runtime.getURL(
			...
			(response) => {
				...
				if (response.status === 400) {
					...
					const tagList =
						this.state.page === 1
							? pageTags
							: this.state.tagList.concat(pageTags);
					this.setState({
	...
	renderContent() {
		...
		this.state.tagList.map((tag) => {
			const Item = new ClockifyTagItem({
				tag,
	...
	selectTag(tag) {
		if (this.manualMode) {
			...
			this.editForm.editTags(tag).then(() => {
