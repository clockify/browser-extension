...
var ClockifyProjectList = class {
	...
	get initialProjectList() {
		...
		return !this.wsSettings.forceProjects &&
			SelectedProject &&
	...
	componentDidMount() {
		aBrowser.storage.local.get('preProjectList', (result) => {
			...
			let { projectList = this.initialProjectList, clientProjects = {} } =
				preProjectList;
	...
	repositionDropDown() {
		...
		if (li) {
			...
			const divDropDown = document.getElementById(
				'divClockifyProjectDropDownPopup'
			);
			divDropDown.style.setProperty(
				'left',
	...
	get HeaderContent() {
		...
		const img = `assets/images/arrow-light-mode${
			this.state.isOpen ? '-up' : ''
		}.png`;
		const str =
			"<div style='display:flex; align-items: center;' tabIndex=0" +
			` class='${
				isOffline
					? 'clockify-project-list-button-offline'
					: this.editForm.isProjectRequired || this.editForm.isTaskRequired
					? 'clockify-project-list-button-required'
					: 'clockify-project-list-button'
			}'>` +
			`<span style='color: ${
			...
			'</span>' +
			'</div>' +
			(this.editForm.isTaskRequired
				? `<div className='clokify-error'>${clockifyLocales.CANT_SAVE_WITHOUT_REQUIRED_FIELDS} (${clockifyLocales.TASK})</div>`
				: '');
		return str;
	}
	...
	onClickedProjectDropDown(el) {
		switch (el.id) {
			...
			case 'clockifyFavoriteStar':
				{
					let li = el.parentNode;
					while (li && li.nodeName !== 'UL') {
						// jump over inner UL/LI
						li = li.parentNode;
					}
			...
			default:
				if (el.nodeName === 'LI' && el.id.startsWith('task_li_')) {
					...
					if (id.endsWith('load_more')) this.loadMoreTasks(el);
					else this.chooseTask(el);
	...
	redrawHeader() {
		const li = this.getElem('#liClockifyProjectDropDownHeader');
		if (li) li.innerHTML = this.HeaderContent;
	...
	open() {
		aBrowser.storage.local.get(['timeEntryInProgress'], (result) => {
			...
			this.getElem('#imgClockifyDropDown').src = aBrowser.runtime.getURL(
	...
	close(fromOtherDropDown) {
		if (fromOtherDropDown && !this.state.isOpen) return;
		this.setState(
			{
				isOpen: false,
			},
	...
	destroy() {
		this.clean();
		if (this.divDropDownPopup) {
			this.divDropDownPopup.removeEventListener(
				'mousewheel',
				this.clockifyMouseWheel,
				true
			);
			this.divDropDownPopup.removeEventListener(
				'DOMMouseScroll',
				...
				if (response.status === 400) {
					...
					if (!filter && page === 1) {
						aBrowser.storage.local.set({
							preProjectList: { projectList: items },
						});
					}
					const projectList =
						this.state.page === 1
							? items
							: this.state.projectList.concat(items);
					this.setState({
						projectList:
	...
	groupByClientName(objectArray) {
		return objectArray.reduce((acc, p) => {
			const key =
				p.client && !!p.client.name ? p.client.name : _clockifyWithoutClient;
			if (!acc[key]) {
				acc[key] = [];
			}
	...
	mapSelectedProject(projectId) {
		...
		const selectedProject = this.state.projectList.find(
					...
					(response) => {
						...
						const projectDB =
							response.data && response.data.length > 0
								? response.data[0]
								: null;
						if (projectDB && !projectDB.archived) {
							// archived should have alerady been checked
							this.setState({
								selectedProject: {
									id: projectDB.id,
									name: projectDB.name,
	...
	render() {
		...
		dropDownPopup.innerHTML =
			...
			` placeholder="${
				this.wsSettings.projectPickerSpecialFilter
					? clockifyLocales.MONKEY_SEARCH
					: clockifyLocales.FIND_PROJECTS
			}"` +
			" class='clockify-list-filter' style='padding: 3px 0px'/>" +
			(!!this.state.filter
				? "<span class='clockify-list-filter__clear' />"
				: '') +
			'</div>' +
			'</div>' +
		...
		this.divDropDownPopup.addEventListener(
			'mousewheel',
			this.clockifyMouseWheel,
			true
		);
	...
	getStar(a) {
		const s = a
			? a.classList.contains('clockify-active')
				...
				: 'normal'
			: 'hover';
		return `url(${aBrowser.runtime.getURL(
			`assets/images/ui-icons/favorites-${s}.svg`
		)})`;
	}

	renderContent() {
		let selectedItem = Object.entries(this.state.Items).find(
			([key, item]) => item.state.isOpen
		);
		if (selectedItem) {
		...
		if (
			...
			clientProjects['NO-PROJECT'].map((project) => {
				const Item =
					selectedItem && selectedItem.project.id === project.id
						? selectedItem
						: new ClockifyProjectItem(project, projectFavorites);
		...
		if (clientProjects['FAVORITES'] && clientProjects['FAVORITES'].length > 0) {
			str +=
				'<li><ul class="clockify-drop-down-2">' +
		...
		sortedClients
			.filter(
				(client) =>
					!['FAVORITES', 'NO-PROJECT', 'WITHOUT-CLIENT'].includes(client)
			)
			.map((client) => {
				str +=
					'<li><ul class="clockify-drop-down-2">' +
	...
	onUpdateFavorite(id, favorite) {
		this.state.projectList.map((p) => {
			if (p.id === id) p.favorite = favorite;
		});
	}
	...
	closeAll() {
		...
		Object.keys(Items).forEach((key) => {
			if (Items[key] && Items[key].state.isOpen) Items[key].close();
		});
	}
	...
	selectTask(task, project) {
		if (this.state.manualMode) {
			...
			this.editForm.editTask(task, project).then((timeEntry) => {
				//this.redrawHeader();
				this.setState({
					selectedProject: project,
					selectedTaskName: task.name, //task.props.name,
