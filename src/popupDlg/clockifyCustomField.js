...
var ClockifyCustomFieldText = class extends ClockifyCustomField {
    ...
    create() {
        const div = document.createElement('div');
        div.setAttribute("id", `${this.divId}`);
        div.setAttribute('class', `clockify-custom-field-ta${this.isDisabled?'-disabled':''}`);
        div.innerHTML =  // data-autoresize 
            `<textarea id='taCustomField${this.index}' index='${this.index}' rows='1' class='custom-field-text${this.isDisabled?'-disabled':''}' title="${this.title}" placeholder="${this.placeholder}" ${this.isDisabled?'disabled':''}>${this.value??''}</textarea>`
        return div;
    }
...
var ClockifyCustomFieldNumber = class extends ClockifyCustomField {
    ...
    create() {
        ...
        div.innerHTML = 
            `<input id='txtCustomField${this.index}' type="number" index='${this.index}' value="${this.value??''}" class='custom-field-number${this.isDisabled?'-disabled':''}' title="${this.title}" placeholder="${this.placeholder}" ${this.isDisabled?'disabled':''}/>`
        return div;
    }
...
var ClockifyCustomFieldLink = class extends ClockifyCustomField {
    ...
    static injectLinkModal() {
        if (document.getElementById('divClockifyLinkModal'))
            return;
...
var ClockifyCustomFieldCheckbox = class extends ClockifyCustomField {
    ...
    draw() {
        ...
        return `<label class="clockify-switch" title="${this.title}">` +
    ...
    create() {
        ...
        div.setAttribute('class', `clockify-custom-field`);
        div.innerHTML = 
            `<div class='clockify-custom-field-inner-checkbox${this.isDisabled?'-disabled':''}'>` +
            this.draw() + 
            "</div>"
        return div;
    }
