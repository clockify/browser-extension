...
var ClockifyProjectItem = class {
		...
		if (!wasOpen) {
			if (this.state.taskList && this.state.taskList.nItems === 0) {
				if (this.state.tasks.length > 0) {
					const li = _clockifyProjectList.getDropDownPopupElem(
						'#li_' + this.project.id
					);
				...
				if (response.status === 400) {
					...
					if (_clockifyProjectList) {
						...
						li.nextSibling
							...
							.forEach((el) => idList.push(el.id));
						if (idList.length) {
							return;
						}
						this.state.taskList.setItems(
							li.nextSibling,
	...
	get itemTasks() {
		...
		return taskCount > 0
			? `<span id='imgClockifyTasksArrow'>${clockifyLocales.TASKS_NUMBER(
					taskCount
			  )}</span>` +
					"<img id='imgClockifyTasksArrow'" +
					` class='clockify-tasks-arrow ${
						isOpen ? 'clockify-down' : 'clockify-right'
					}'` +
					` src='${aBrowser.runtime.getURL(
						`assets/images/filter-arrow-${isOpen ? 'down' : 'right'}.png`
					)}'` +
					" alt='Expand' />"
			: '<span></span>';
	}
	...
	get InnerHTML() {
		const { projectId, name, isOpen, favorite, client, projectFavorites } =
			this.state;
		let title = name;
		let clientName = '';
		...
		let str = `<ul class='clockify-project-item' title="${title}">
                <li class='clockify-project-item-dot' style='background: ${
									this.project.color
								}'></li>
                <li class='clockify-project-item-name' id='clockifyProjectItemName' tabIndex='0' title="${title}">
                    ${name} ${clientName}
                </li>
                <li class='clockify-project-item-tasks' title=${
									clockifyLocales.EXPAND
								}>${this.itemTasks}</li>
                ${
									projectFavorites
										? `<li class='clockify-project-item-favorite' title=${
												clockifyLocales.FAVORITE
										  }>
                        ${
													projectId !== 'no-project'
														? `<a id='clockifyFavoriteStar'
                                class="clockify-dropdown-star ${
																	favorite
																		? 'clockify-active'
																		: 'clockify-normal'
																}" 
	...
	renderContent() {
		const el = _clockifyProjectList.getDropDownPopupElem(
			'#li_' + this.project.id
		);
	...
	clean() {
		if (this.state.taskList) this.state.taskList.clean();
	}
};
