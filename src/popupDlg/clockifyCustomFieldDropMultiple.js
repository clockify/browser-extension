...
var ClockifyCustomFieldDropMultiple = class extends ClockifyCustomField {
	...
	onClickedCFPopup(target) {
		if (
			this.state.isOpen &&
			this.divDropDownPopup &&
			this.divDropDownPopup.contains(target)
		) {
			const that = this;
			setTimeout(() => {
	...
	repositionDropDown() {
		if (this.divDropDownPopup) {
			...
			if (li) {
				...
				this.divDropDownPopup.style.setProperty(
					'left',
	...
	get HeaderContent() {
		...
		let title =
			this.title +
			(this.value.length === 0 ? '' : this.value.length === 1 ? ': ' : ':\n');
		const className = `clockify-tag-list-button${
			isOffline || this.isDisabled ? '-disabled' : ''
		}`;

		const arr = this.value.map((name, index, list) => {
			name = this.encoded(name);
			title += name + '\n';
			return (
				"<span className='clockify-tag-list-selected-item'>" +
				name +
				(index < list.length - 1 ? ', ' : '') +
				'</span>'
			);
	...
	open() {
		aBrowser.storage.local.get(['timeEntryInProgress'], (result) => {
			...
			this.getImg().src = aBrowser.runtime.getURL(
	...
	render() {
		...
		dropDownPopup.innerHTML =
			"<ul class='clockify-drop-multiple clockify-tag-list'>" + '</ul>';
	...
	renderContent() {
		...
		this.state.items.map((item) => {
			const Item = new ClockifyMultipleItem({
				pDrop: this,
				item,
				isChecked: this.value.includes(item.name),
			});
	...
	selectItem(id) {
		...
		Object.keys(Items).forEach((key) => {
			...
			if (multipleItem.state.isChecked) values.push(multipleItem.item.name);
		});
		this.value = values;
		this.redrawHeader();
	}
