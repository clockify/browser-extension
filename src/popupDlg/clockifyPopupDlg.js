...
var ClockifyPopupDlg = class {
	...
	create(timeEntry, msg, manualMode) {
		...
		_clockifyProjectList = new ClockifyProjectList(
			this.editForm,
			timeEntry,
			manualMode
		);
		_clockifyTagList = new ClockifyTagList(
			this.editForm,
			timeEntry,
			manualMode
		);
	...
	get isOwnerOrAdmin() {
		return this.userRoles.some((row) =>
			['WORKSPACE_ADMIN', 'WORKSPACE_OWN'].includes(row.role)
		);
	}

	getWSCustomField(customField) {
		return this.wsCustomFields.find(
			(cf) => cf.id === customField.customFieldId
		);
	}
	...
	onClickedLinkModal(divClockifyLinkModal, target) {
		...
		try {
			this.editForm.customFields[index].onClickedLinkModal(
				this.editForm.divCustomFields,
				divClockifyLinkModal,
				target
			);
		} catch (e) {
			console.error(e);
	...
	onClickedProjectDropDown(el) {
		...
		switch (el.id) {
			...
			default:
				if (
					el.nodeName === 'LI' &&
					(el.id.startsWith('task_li_') || el.id.startsWith('li_'))
				) {
					_clockifyProjectList.onClickedProjectDropDown(el);
				}
				break;
	...
	closeAllDropDowns() {
		if (_clockifyProjectList) _clockifyProjectList.close(true);
		if (_clockifyTagList) _clockifyTagList.close(true);
...
aBrowser.runtime.sendMessage(
	{ eventName: 'takeTimeEntryInProgress' },
	(response) => {
